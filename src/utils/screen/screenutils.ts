...
export const safeArea = {
  top: 44, // Standard status bar + notch
...
export const metrics = {
  ...
  /**
   * Clamps a value between minimum and maximum responsive values.
   * @param {number} value - The base value to clamp.
   * @param {number} min - Minimum value after scaling.
   * @param {number} max - Maximum value after scaling.
   * @returns {number} The clamped responsive size.
   *
   * Use for:
   * - Preventing UI elements from becoming too small or too large
   * - Setting bounds on responsive scaling
   * - Ensuring accessibility compliance
   */
  clamp: (value: number, min: number, max: number) => {
    const scaled = Math.round(SCALE * value * DENSITY_SCALE);
    return Math.max(min, Math.min(max, scaled));
  },

  /**
   * Scales based on the smallest dimension (width or height).
   * @param {number} size - The size to scale.
   * @returns {number} The scaled size.
   *
   * Use for:
   * - Square components that should maintain aspect ratio
   * - Icons that need consistent sizing regardless of device orientation
   * - Circular elements
   */
  square: (size: number) => {
    const minScale = Math.min(SCALE, VERTICAL_SCALE);
    return Math.round(minScale * size * DENSITY_SCALE);
  },
};

export const { s, vs, ms, mvs, font, w, h, px, clamp, square } = metrics;
